...
#
# Usage:
#   bash run_twistb_4calc.sh -n N -m M [-l]
...
while getopts "n:m:l" opt; do
    case $opt in
        ...
        m) M=$OPTARG ;;
        l) LOCAL=1   ;;
        *) echo "Usage: $0 -n N -m M [-l]" >&2; exit 1 ;;
    esac
done
...
# ── share neighbor list with calcs 2-4 ───────────────────────────────────────
...
if [[ -n "$SAVE_DIR_1" && -f "${SAVE_DIR_1}/neighbors.mat" ]]; then
    ...
    echo "--> Sharing neighbor list (${OUTNAME}.save/neighbors.mat) with calcs 2-4"
    for calc in cond_bands val_wfc cond_wfc; do
        mkdir -p "${WDIR}/${calc}/${OUTNAME}.save"
        cp "${SAVE_DIR_1}/neighbors.mat" "${WDIR}/${calc}/${OUTNAME}.save/"
    done
    READ_NB=true
else
    echo "WARNING: neighbors.mat not found after calc 1 — calcs 2-4 will recompute" >&2
    READ_NB=false
fi
